...
app.get("/search", async (req, res) => {
    ...
    if (!books || books.length === 0) return res.status(404).json({Error: "Book not found, try different one!"});
...
app.get("/ebook", async (req, res) => {
    ...
    const {url} = req.query;
    if (!url) return res.status(400).json({error: "Missing query 'q"});

    const downloadUrl = await getDownloadLink(url);
    if (!downloadUrl) return res.status(404).json({Error: "Ebook not found, try different one!"});
